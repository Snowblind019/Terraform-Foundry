# 04 - HashiCorp Vault and the Vault Provider
...
1. A `vault` provider block with the `address` of the running Vault instance. In the demo
   that is `http://127.0.0.1:8200`, Vault running locally on its default port.
2. A `vault_generic_secret` data source pointing at the path where the secret lives,
   `secret/db_creds`.
3. An output, `vault_secrets`, referencing `data_json` on that data source, with `sensitive`
   set, so the fetch can be confirmed without printing the value.
